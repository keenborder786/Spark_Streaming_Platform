...
class SparkProcessing(SparkJob):
    ...
    def table_processing(self,df:pyspark.sql.DataFrame , 
        ...
        fields = json.loads(sink_schema.json())['fields']## Get the sink schema as json
        all_names_fields = []## This is needed to select the relevant columns
        for field in fields:
            name,schema = field['name'],field['type']## Get the name of column and type of column
            df = df.withColumn(name , when(df.op != 'd' ,(col(f'payload.after.{name}.value'))).otherwise(col(f'payload.before.{name}.value')))## If operation is u or c then take after values otherwise take the before values
            if schema == 'timestamp': df = df.withColumn(name ,col(name)/1000.0) ## We need to  avoid the ValueError: year is out of range
            df = df.withColumn(name , col(name).cast(spark_to_python_types[schema]))## Cast to sink schema, where spark_to_python_types convert python types to Spark Types
