...
class SparkJob:
    """
    The main class which creates the spark session and reads the data from kafka
    """
    ...
    def get_spark_session(
        self, app_name: str, hadoop_config: Optional[Dict] = {}
    ...
    def read_kafka_stream(
        ...
        for k, v in kafka_config.items():
            df = df.option(k, v)  # Loading the other specific config
        df = df.load()
