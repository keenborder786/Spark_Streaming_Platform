...
class DeltaLakeInteraction:
    ...
    def create_delta_table(
        self, schema: pyspark.sql.types.StructType, table_properties: Dict, *partition_col
    ...
    def restore_table_version(self, version_number: int) -> None:
        ...
        deltaTable = DeltaTable.forPath(
            self.spark_session, "s3a://{}/{}".format(
                self.bucket, self.table_name)
        )
        deltaTable.restoreToVersion(version_number)
        print("Table has been restored to the following version {}".format(
            version_number))
    ...
    def restore_table_time_period(self, time_period: str) -> None:
        ...
        print("Table has been restored to the following timestamp {}".format(time_period))
    ...
    def query_table_time_period(self, time_period: str) -> pyspark.sql.DataFrame:
        ...
        df = (
            ...
            .option("timestampAsOf", time_period)
            .load("s3a://{}/{}".format(self.bucket, self.table_name))
        )
        return df
    ...
    def query_latest_table(self) -> pyspark.sql.DataFrame:
        ...
        df = self.spark_session.read.format("delta").load(
            "s3a://{}/{}".format(self.bucket, self.table_name)
        )
        return df
